...
try {
  ...
  const songs = await pool.query("SELECT DISTINCT s.song_name AS name, s.artist_f_name AS artist, sl.song_img AS image, sl.song_link AS path FROM songs AS s, song_link as sl WHERE s.song_id = sl.song_id")
